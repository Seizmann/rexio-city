...
export default function PostCard({ post, onUpdate }: PostCardProps) {
  ...
  const handleAction = async (
    ...
    const countKey = actionType === 'like' ? 'likes' : actionType === 'repost' ? 'reposts' : 'bookmark_count';
    const currentCount = Number(localPost[countKey as keyof Post]) || 0;

    // Optimistic update
    const updatedPost = {
      ...localPost,
      [`is_${actionType}d`]: !isCurrentlyActive,
      [countKey]: isCurrentlyActive ? Math.max(0, currentCount - 1) : currentCount + 1,
    };
  ...
  const handleCommentAdded = () => {
    const updatedPost = {
      ...localPost,
      comments: (localPost.comments ?? localPost.comment_count ?? 0) + 1,
      comment_count: (localPost.comment_count ?? localPost.comments ?? 0) + 1,
    };
    setLocalPost(updatedPost);
    if (onUpdate) onUpdate(updatedPost);
  };
